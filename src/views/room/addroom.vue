    ...
    <el-dialog :title="title" :visible.sync="visible" :lock-scroll="false" :show-close="false" :close-on-click-modal="false">
      <el-form ref="roomForm" :model="item" :rules="rules" label-width="120px">
        ...
        <el-form-item label="客房类型：" prop="roomType">
          <el-select v-model="item.roomType" placeholder="请选择客房类型" clearable style="width:100%;">
            <el-option v-for="item in roomTypeList" :key="item" :label="item" :value="item">
            </el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="客房状态：">
          <el-select v-model="item.state" placeholder="请选择客房状态" clearable style="width:100%;">
      ...
      <span slot="footer">
        ...
        <el-button @click="resetForm('roomForm')">取消</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
<script>
import { getCategoryList, getAllRoomTypeList } from '@/api/room';
